...
class UserData {
public:
	static u_int32_t toKey(string const& id) throw();

#define GET_AND_SET(type, Type) \
	public: \
	void set##Type(u_int32_t key, type value) throw() { type##Map[key]; }; \
	type const& get##Type(u_int32_t key) const throw() { \
	...
	private: \
	typedef map<u_int32_t, type> Type##Map; \
	Type##Map type##Map;
